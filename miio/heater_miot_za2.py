...
class LedBrightness_za2(enum.Enum):
    Bright = 0
    Off = 1
    Dim = 2
...
class HeaterMiotStatus_za2(DeviceStatus):
    """Container for status reports from the Xiaomi Smart Space Heater 1S."""

    def __init__(self, data: Dict[str, Any]) -> None:
        """
        Response (MIoT format) of Xiaomi Smart Space Heater 1S (zhimi.heater.za2).
        """
        self.data = data
    ...
    @property
    def led_brightness(self) -> LedBrightness_za2:
        """LED indicator brightness."""
        return LedBrightness_za2(self.data["led_brightness"])


class HeaterMiot_za2(MiotDevice):
    """Main class representing the Xiaomi Smart Space Heater 1S (zhimi.heater.za2)."""
    ...
    def status(self) -> HeaterMiotStatus_za2:
        """Retrieve properties."""

        return HeaterMiotStatus_za2(
            {
    ...
    @command(
        click.argument("brightness", type=EnumType(LedBrightness_za2)),
        default_output=format_output(
            "Setting LED indicator brightness to {brightness}"
        ),
    )
    def set_led_brightness(self, brightness: LedBrightness_za2):
        """Set led brightness."""
