...
PROSONIC_POWER_ON_PRONTO = '0000006C00220002015B00AD001600160016001600160016' \
...
class TestChuangmiIr(TestCase):
    ...
    def test_play_raw(self):
        assert self.device.play_raw(PROSONIC_POWER_ON) is True
        assert self.device.play_raw(PROSONIC_POWER_ON, 19200) is True

    def test_play_pronto(self):
        assert self.device.play_pronto(PROSONIC_POWER_ON_PRONTO) is True
        assert self.device.play_pronto(PROSONIC_POWER_ON_PRONTO, 0) is True
        assert self.device.play_pronto(PROSONIC_POWER_ON_PRONTO, 1) is True

        with pytest.raises(ChuangmiIrException):
            self.device.play_pronto(PROSONIC_POWER_ON_PRONTO, -1)
