...
class AirConditionerMiot(MiotDevice):
    ...
    def on(self):
        """Power on."""
        return self.set_property("power", "on")

    @command(default_output=format_output("Powering off"))
    def off(self):
        """Power off."""
        return self.set_property("power", "off")
    ...
    @command(
        ...
        default_output=format_output("Setting timer to {enabled},{minutes},0"),
    )
    def set_timer(self, enabled, minutes):
        """Set timer."""
        if not enabled:
            minutes = 0

        elif enabled and minutes is None:
            raise AirConditionerMiotException("Timer minutes not set")

        return self.set_property(
            "timer", ",".join([str(int(enabled)), str(minutes), "0"])
        )

    @command(
        click.argument("clean", type=bool),
        default_output=format_output("Setting clean to {clean}"),
    )
